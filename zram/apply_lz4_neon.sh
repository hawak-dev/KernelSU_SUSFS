...
# Replaces original 4 version-specific patch files, using regex matching instead of fixed line numbers
#
...
# Pattern consistent: int out_len = LZ4_decompress_safe(src, dst, slen, *dlen);
for file in crypto/lz4.c crypto/lz4hc.c; do
  if [ ! -f "$file" ]; then
    echo "Skipped (does not exist): $file"; ((SKIPPED++)) || true; continue
  fi
...
if [ ! -f "$file" ]; then
  echo "Skipped (does not exist): $file"; ((SKIPPED++)) || true
